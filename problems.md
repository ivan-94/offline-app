...
3.  拿到 html 页面后，浏览器开始渲染流程。因为我们使用前端渲染的方式，只有等到所有‘必须’的资源加载完毕之后，用户才能看到非空白页面。这个阶段才是前端能控制的‘主要战场’, 也是这次预研的主题
    ...
    按照最佳实践，单个文件的体积，应该控制在 250kb 以内，上图可以看到文件基本控制在 250k 左右，体积最大的是`ionic.min.js`达到 400k. 在开启 gzip 后，可以极大缩小传输体积.

    虽然体积还可以在缩小，但需要平衡开发效率和运行性能, 比如使用扫码登录就使用了`preact`, 这个精简版的`React`体积非常小，但是运行性能或者生态，是无法和`React`比较的。

    通常一个资源的下载过程会经历连接建立，请求处理，响应:

    ![download](attachments/download.png)

    * 第一部分表示发起请求到连接建立的时间;
    * 第二部分表示服务器处理的时间;
    * 第三部分表示服务器写入数据到连接，到客户端接收完毕的时间。

    对于一个资源下载而言，这三个阶段都比较重要。这主要依赖于网络和服务器的负载, 资源缓存情况。
    至于服务器性能对资源下载的影响，还需要严谨的基准测试比较。下图只供参考:

    ![server bench](attachments/server-bench.png)

    (这是 2014年的一个数据，伺服12字节的静态资源)

    影响这个阶段页面加载的因素有很多, 主要因素是资源的体积大小和分配情况. 总结这个阶段我们能做些什么?
...
3.  静态资源伺服可能还有优化空间(需要基准测试支持).
    目前应用使用 Java 的 内嵌服务器 Jetty 服务器进行静态资源的伺服, 一般适用于`开发环境`, 以支持快速开发. 由上面的例子中也可以看到, 静态资源连接的建立,响应的延迟都比较长,且不稳定, 而相比而言, Nginx 更适合静态资源的伺服.

    实现动静分离可以优化 Java 服务器性能和内存占用, 让服务器更专注于业务的处理. 另外也可以提高静态资源的下载效率
...
* [18 Free Web Servers Performance Benchmark - (2014-4-9)](http://benchmark.ecmscloud.com/web-servers-performance-benchmark)
